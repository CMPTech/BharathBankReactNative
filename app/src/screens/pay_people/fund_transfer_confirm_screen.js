...
export default function ConfirmPaymentScreen({ navigation, route }) {
    ...
    const paymentVerifyCall = useCallback(async () => {
        try {
            ...
            setLoading(true);
            const response = await Home.FTVerifyApiCall(request);
            setAuthPriorityEnabled(response.authPriorityEnabled);
            if (response.otpEnabled) {
                navigation.navigate(PAY_PEOPLE.PAY_PEOPLE_OTP_SCREEN, { requestData: params.requestFT })
            }
            else {
                fTConfirm();
            }
