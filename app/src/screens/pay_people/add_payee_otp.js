...
export default function AddPayeeOTPScreen({ navigation, route }) {
  const { params } = route;
  ...
  const nextStep = useCallback(async (data) => {
    try {
      ...
      navigation.navigate(PAY_PEOPLE.PAY_PEOPLE_SUCCESS, {
        message: `${params.payeeName} ${'Added sucessfully'}`, subMessage: `${'You can now pay'} ${params.payeeName} ${'with ease whenever you want.'}`, Payee: {
            "activeStatus": "Y",
