...
const tabs_list = [
    ...
    },
    {
        id: 2,
        label: screens.neft,
        value: i18n.t(AUTH_KEYS.PAY_PEOPLE.PAY_NEFT)
    },
    {
        id: 3,
        label: screens.imps,
...
export default function PayeeHistory({ navigation, route }) {
    const { params } = route;
    ...
    const onViewChangeRef = useRef(({ viewableItems, changed }) => {
        if (viewableItems.length > 0) {
            ...
            setHasNext(true);
            setHistoryList([]);
            getHistorytList(viewableItems[0].item.value);
        }
    })
    ...
    useEffect(() => {
        getHistorytList(transferType);
    }, [])
    const getHistorytList = useCallback(async (type) => {
        try {
            ...
            if (hasNext) {
                ...
                setLoading(true);
                const response = await Home.getHistoryListApi(request);
            ...
            <View style={{
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={{
                        ...
                        padding: 10
                    }}>{t(AUTH_KEYS.PAY_PEOPLE.PAYEMENT_HISTORY)}</Text>
    ...
    const renderItem = ({ item, index }) => (
        ...
        >
            <View style={{ flexDirection: "row", alignItems: 'center', marginTop: 10, marginLeft: 20, marginRight: 10 }}>
                <Text style={{ color: theme.colors.grey, width: '45%' }}>{moment(new Date(item.txnDate)).format('DD-MM-YYYY')}</Text>
