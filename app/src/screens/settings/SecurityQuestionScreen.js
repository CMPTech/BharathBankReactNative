...
import { CALL_US, HOME, } from '../../routes';
...
export default function SecurityQuestionScreen({ navigation, route }) {
  ...
  const getSecurityQuestion = useCallback(async () => {
    try {
      ...
      setLoading(true);
      const response = await Auth.getSecurityQuestionApi(request);
      setLoading(false);
      setFaqsList(response.securityList)
      setRawFaqsList(response.securityList)
    } catch (error) {
      setLoading(false);
      showMessage({
  ...
  const setSecurityQuestion = useCallback(async () => {
    try {
      ...
      navigation.navigate(HOME.CUSTOM_DRAWER, { profiledata: profiledata,loginResponse: loginResponse })
            ...
            >
              ...
              <View style={{ position: 'absolute', right: 20, top: 0, bottom: 0, justifyContent: 'center' }}>
                {item?.answer?.length >= 1 &&
                  <TickIcon color={theme.colors.buttonStrokeStartColor} />
                }
              </View>
  ...
  return (
    ...
    >
      ...
      <Text style={{ ...FONTS.h3, color: theme.colors.textColor, textAlign: 'left', marginLeft: 20, marginTop: 20, width: '95%', lineHeight: 20 }}>{"Please answer atleast 3 questions"}</Text>
      <Text style={{ ...FONTS.body3, color: theme.colors.grey, textAlign: 'left', marginLeft: 20, marginTop: 10, marginBottom: 20, width: '95%', lineHeight: 20 }}>{"Please refrain from using spaces or special characters in your answers"}</Text>
      ...
      <View style={{ position: 'absolute', bottom: 0, width: '100%', flexDirection: 'row', justifyContent: 'space-between' }}>
        <MainButton
          noBorder
          btnContainerStyle={{ width: loginResponse.skipSecurityQuestions ? '49%' : '100%' }}
        ...
        />
        {loginResponse.skipSecurityQuestions &&
          <MainButton
            noBorder
            btnContainerStyle={{ width: '49%' }}
            title={"Skip"}
            onPress={() => {
              navigation.navigate(HOME.CUSTOM_DRAWER, { profiledata: profiledata,loginResponse: loginResponse })
            }}
